...
//        build a normal horizontal cell
        let imageOfCurrent = self.images[indexPath.row]
        if imageOfCurrent.size.height > defaultWidthOfItem {
            
            let sizeOfImage = imageOfCurrent.size
            let size = CGSizeMake(defaultWidthOfItem, defaultWidthOfItem * sizeOfImage.width/sizeOfImage.height)
...
//        tranfsorm to display 3d effect
        ...
        if distance > distanceLimitted{
            ...
            if let cell = self.collectionView?.cellForItemAtIndexPath(indexPath){
                if let _ = self.collectionView!.viewWithTag(indexPath.row) {
